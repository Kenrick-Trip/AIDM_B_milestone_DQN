...
class MazeView2D:
    ...
    def __init__(self, maze_name="Maze2D", maze_file_path=None,
        ...
        self.__entrance = np.asarray(self.maze_size) // 2
    ...
    def update_shortest_path(self, shortest_path, mode="human", timestamp=None):
        try:
            if not timestamp:
                timestamp = str(datetime.now().strftime("%Y%m%d_%H-%M-%S"))
            img_output = self.__view_update_shortest_path(shortest_path, mode)
            pygame.image.save(self.maze_layer, "output_files/shortest_path" + timestamp + ".png")
            self.__controller_update()
        except Exception as e:
            self.__game_over = True
            self.quit_game()
    ...
    def update_milestones(self, milestones, mode="human", timestamp=None):
        try:
            if not timestamp:
                timestamp = str(datetime.now().strftime("%Y%m%d_%H-%M-%S"))
    ...
    def reset_robot(self):
        ...
        self.__robot = np.asarray(self.maze_size) // 2
        self.__draw_robot(transparency=255)
    ...
    def __view_update_shortest_path(self, shortest_path, mode="human"):
        if not self.__game_over:
            # update the robot's position
            self.__draw_entrance()
            self.__draw_goal()
            self.__draw_portals()
            self.__draw_robot()
            for v in shortest_path[1:-1]:
                self.colour_cell(v, colour=(150, 150, 150), transparency=135)
    ...
    def __view_update_milestones(self, milestones, mode="human"):
        if not self.__game_over:
            # update the robot's position
            self.__draw_entrance()
            self.__draw_goal()
    ...
    def __draw_robot(self, colour=(255, 102, 255), transparency=255):
    ...
    def __draw_entrance(self, colour=(0, 0, 150), transparency=235):
        ...
        self.colour_cell(self.entrance, colour=colour, transparency=transparency)

    def __draw_goal(self, colour=(150, 0, 0), transparency=235):

        self.colour_cell(self.goal, colour=colour, transparency=transparency)
    ...
    def __draw_portals(self, transparency=160):
        ...
        for portal in self.maze.portals:
            ...
            for location in portal.locations:
                self.colour_cell(location, colour=colour, transparency=transparency)

    def colour_cell(self, cell, colour, transparency):
    ...
    def __draw_coordinates(self):
        ...
        for y in range(self.maze.MAZE_H):
            for x in range(self.maze.MAZE_W):
                # text = f"{(x, y)}"
                text = str(self.__maze.maze_cells[x, y])
...
class MazeBuilder:
    ...
    def set(self, x, y, val):
        self.maze_cells[x, y] = val

    def create_custom_10x10(self):
        for y, row, in enumerate([
            [2, 6, 10, 12, 4, 4, 2, 12, 2, 12],
            [4, 3, 8, 5, 7, 11, 8, 5, 6, 9],
            [5, 2, 14, 13, 7, 10, 10, 13, 3, 12],
            [3, 12, 5, 5, 7, 12, 6, 13, 4, 5],
            [4, 5, 1, 5, 1, 5, 1, 7, 13, 5],
            [7, 11, 10, 11, 10, 15, 10, 13, 1, 5],
            [3, 10, 8, 6, 8, 5, 2, 15, 10, 13],
            [4, 6, 14, 15, 10, 11, 12, 1, 2, 13],
            [5, 5, 5, 7, 10, 8, 1, 6, 10, 9],
            [3, 9, 1, 1, 2, 10, 10, 1, 10, 8]
                                  ]):
...
class Maze:
    ...
    def __init__(self, maze_cells=None, maze_size=(10, 10), has_loops=True, num_portals=0):
        ...
        # mzb = MazeBuilder(maze_size)
        # mzb.create_custom_10x10()
        # self.maze_cells = mzb.maze_cells
        # np.save("maze2d_10x10_custom.npy", self.maze_cells)
