...
class MountainCarMilestonesEnv(gym.Env):
    """
    ### Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def render(self, mode="human", milestones=None, render_ms=False):
        try:
            import pygame
        ...
        if render_ms:
            ...
            if num_m > 1:
                ...
                milestone_locations = [milestones[x].goal_state[0] for x in range(num_m)]
                milestone_locations = milestone_locations[:num_m-1]
                for mx in milestone_locations:
                    flagmx = int((mx - self.min_position) * scale)
                    flagmy1 = int(self._height(mx) * scale)
                    flagmy2 = flagmy1 + 12
                    gfxdraw.vline(self.surf, flagmx, flagmy1, flagmy2, (0, 0, 0))

                    gfxdraw.aapolygon(
                        self.surf,
                        [(flagmx, flagmy2), (flagmx, flagmy2 - 3), (flagmx + 5, flagmy2 - 1.5)],
                        (204, 0, 0),
                    )
                    gfxdraw.filled_polygon(
                        self.surf,
    ...
    def render_milestones(self, milestones, mode="human", close=False, timestamp=None):
        ...
        return self.render(mode, milestones, True)
